...
//   protoc               v5.29.3
// source: src/Protos/healthData.proto
...
export interface HealthData {
  CreateHealthRecord(request: HealthRecordRequest): Promise<Empty>;
  GetHealthRecord(request: HealthRecordIdRequest): Promise<HealthRecord>;
  GetAllHealthRecords(request: Empty): Promise<HealthRecordResponse>;
  UpdateHealthRecord(request: HealthRecord): Promise<Message>;
  DeleteHealthRecord(request: HealthRecordIdRequest): Promise<Message>;
  GetAthleteHealthRecords(request: AthleteFilter): Promise<HealthRecordResponse>;
}

export const HealthDataServiceName = "HealthData";
export class HealthDataClientImpl implements HealthData {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || HealthDataServiceName;
    this.rpc = rpc;
    this.CreateHealthRecord = this.CreateHealthRecord.bind(this);
    this.GetHealthRecord = this.GetHealthRecord.bind(this);
    this.GetAllHealthRecords = this.GetAllHealthRecords.bind(this);
    this.UpdateHealthRecord = this.UpdateHealthRecord.bind(this);
    this.DeleteHealthRecord = this.DeleteHealthRecord.bind(this);
    this.GetAthleteHealthRecords = this.GetAthleteHealthRecords.bind(this);
  }
  CreateHealthRecord(request: HealthRecordRequest): Promise<Empty> {
    const data = HealthRecordRequest.encode(request).finish();
    const promise = this.rpc.request(this.service, "CreateHealthRecord", data);
    return promise.then((data) => Empty.decode(new BinaryReader(data)));
  }

  GetHealthRecord(request: HealthRecordIdRequest): Promise<HealthRecord> {
    const data = HealthRecordIdRequest.encode(request).finish();
    const promise = this.rpc.request(this.service, "GetHealthRecord", data);
    return promise.then((data) => HealthRecord.decode(new BinaryReader(data)));
  }

  GetAllHealthRecords(request: Empty): Promise<HealthRecordResponse> {
    const data = Empty.encode(request).finish();
    const promise = this.rpc.request(this.service, "GetAllHealthRecords", data);
    return promise.then((data) => HealthRecordResponse.decode(new BinaryReader(data)));
  }

  UpdateHealthRecord(request: HealthRecord): Promise<Message> {
    const data = HealthRecord.encode(request).finish();
    const promise = this.rpc.request(this.service, "UpdateHealthRecord", data);
    return promise.then((data) => Message.decode(new BinaryReader(data)));
  }

  DeleteHealthRecord(request: HealthRecordIdRequest): Promise<Message> {
    const data = HealthRecordIdRequest.encode(request).finish();
    const promise = this.rpc.request(this.service, "DeleteHealthRecord", data);
    return promise.then((data) => Message.decode(new BinaryReader(data)));
  }

  GetAthleteHealthRecords(request: AthleteFilter): Promise<HealthRecordResponse> {
    const data = AthleteFilter.encode(request).finish();
    const promise = this.rpc.request(this.service, "GetAthleteHealthRecords", data);
    return promise.then((data) => HealthRecordResponse.decode(new BinaryReader(data)));
  }
}
