...
import { CreateHealthRecord, DeleteHealthRecord, GetAllHealthRecords, GetAthleteHealthRecords, GetHealthRecords, UpdateHealthRecord } from '../services/healthDataService';
...
router.get('/record/athlete/:athleteId/:activityStatus/:startTime/:endTime', (_req, res) => {
  GetAthleteHealthRecords({
    ...
    startTime: _req.params.startTime,
    endTime: _req.params.endTime
  })
    .then((records) => res.status(200).json(records))
    .catch((error) => res.status(500).json({ error: error.message }));
});
